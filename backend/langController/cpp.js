...
const cpp = (fileName, input, res) => {
  ...
  fs.writeFile(`${fileName}.txt`, input, (err) => {
    ...
    exec(`docker run -d -it cpp:v1 sh`).then((response) => {
      ...
      exec(
        `docker cp ${fileName}.cpp ${container_id}:/usr/cpp && docker cp ${fileName}.txt ${container_id}:/usr/cpp && docker exec -t ${container_id} sh -c "g++ ${fileName}.cpp -o ./a && ./a<${fileName}.txt"`
      )
      .then((resp) => {
        console.log(resp);
